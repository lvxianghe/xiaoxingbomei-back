...
public class ChatClient_Utils
{
     ...
     * @return 对话结果
     */
    public static Object chatWithDefaultSystem(ChatClient chatClient, String chatId, String prompt, String systemPrompt, List<Advisor> additionalAdvisors, boolean isStream)
    {
        try {
            // 验证静态实例是否已初始化
            ...
            // 构建新的ChatClient，设置defaultSystem和defaultAdvisors
            List<Advisor> advisors = buildAdvisors(additionalAdvisors);
            ...
            // 构建新的ChatClient
            ChatClient clientWithDefaultSystem = builder.build();
            
            Object result;
            if (isStream)
            {
                result = clientWithDefaultSystem.prompt()
                        .user(prompt)
                        .advisors(advisors)
                        .advisors(a -> a.param(AbstractChatMemoryAdvisor.CHAT_MEMORY_CONVERSATION_ID_KEY, chatId))
                        .stream()
